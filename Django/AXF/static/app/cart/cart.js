$(function () {
    ...
    $('.add').click(function () {
        ...
        var cartid = $(this).parents('.menuList').attr('cartid');  # $(this).parents().find('.menuList').attr('cartid')验证不能这样写
        ...
        $.get('/app/addnum/', {'cartid': cartid}, function(data) {
            ...
            if (data.status == 1){
                $(that).prev().html(data.num)   # 里面不能写this,里面的this是一个对象,谁调就是谁
            }                                   # 输入框才用val,它是{{cart.num}}
            else {
    ...
    $('.select').click(function () {
        ...
        $.get('/app/cartselect/', {cartid: cartid}, function(data) {
            ...
            if (data.status == 1) {
                ...
                // }
                // else {
                //     $(that).find('span').html('');# 如果没",则是获取,所以需写"".
                // }
    ...
    $('#allselect').click(function () {
        ...
        $('.menuList').each(function() {
            ...
            if (select){
                //如果是勾选的，则添加到selects中
                selects.push($(this).attr('cartid'))  # 通过属性获得cartid
            }
            else {
    ...
    // 检测是否全选了,很多地方都需要调,所以写这个函数,js遵循驼峰原则
    isAllSelected();   # 每次进来的时候就需要调用,js中函数可以写在定义的前面
    ...
    function calculate() {
        ...
        // 显示总价
        $('#totalprice').html(total.toFixed(2));  # toFixed保留两位小数
    }
    ...
    $('#calculate').click(function () {
        ...
        $.get('/app/orderadd/', function(data) {
            // console.log(data)
            if (data.status == 1){
                location.href = '/app/order/' + data.orderid + '/'   # 添加orderid参数进order页面
            }
            else {
                console.log(data.msg)
            }
